...
window.addEventListener('load', async () => {
  ...
  /*if (localStorage['ak-referred']) {
    ...
    // localStorage.removeItem('ak-referred'); // user will always be referred
  ...
  */
  if (Clerk.user) { // logged-in user retrieve from DB
    ...
    if (!data && localStorage['ak-referred']) {
      localStorage.removeItem('ak-referred');
      showErrorNRedirect(userMail); 
      return;
    }
    else if (!data) {
      return;
    ...
    if (referrerMail) {
      console.log('Referred user!')
      const referrerData = await retrieveReferrerAttractions(referrerMail);

      const $selectPlanList = document.querySelector('[data-ak="select-plan-list"]');
      
      /*let { tripName:ownerTripName } = data;
      const ownerLinkName = ownerTripName ? `${ownerTripName.split(/\s+/)[0]} - ${userMail} (me)` : `${userMail} (me)`;
      setupSelectPlanList($selectPlanList, ownerLinkName);

      let { tripName:referrerTripName } = referrerData;
      const referrerLinkName = referrerTripName ? `${referrerTripName.split(/\s+/)[0]} - ${referrerMail}` : referrerMail;
      setupSelectPlanList($selectPlanList, referrerLinkName);*/
      ...
      const parsedSavedAttractions = JSON.parse(savedAttractions);
      restoreSavedAttractions(parsedSavedAttractions);
    }
  }
  ...
  function showErrorNRedirect(userMail) {
...
async function retrieveReferrerAttractions(mail) {
  // const data = await retrieveDBData(userMail); 
  ...
  // console.log('Main Email:', mail)
  console.log('Referrer Email:', mail)
...
async function saveAttractionsDB() {  
    ...
    if (!tripName || !travelDates || !savedAttractions) return;

...
$saveMailBtn.addEventListener('click', async e => {
  ...
  const mailArr = [];
  const inputs = [...$secondaryMailWrap.querySelectorAll('.ak-add-mail-input')];
  for (const inp of inputs) {
    const mail = inp.value.trim();
    if (!mail) {
      inp.classList.add('active');
      setTimeout(()=>inp.classList.remove('active'),1000);
      return;
    }
    mailArr.push(mail);
  }
...
async function saveMailsDB(mailArr) {  
  ...
  for await (const mail of mailArr) {
    const userRef = doc(db, 'locationsData', `user-${mail}`);
    const saveObj = {
...
async function retrieveReferrerAttractions(userMail) {
  ...
  const referrerRef = doc(db, 'locationsData', `user-${referrerMail}`);
  const referrerSnap = await getDoc(referrerRef);

  if (!referrerSnap.exists()) {
    // referrerSnap.data() will be undefined in this case
    console.log('No user with referrer email:', referrerMail);
    return; 
  }  

  const referrerData = await docSnap.data(); 
  const { secondaryMail, savedAttractions } = referrerData;

  if (userMail !== secondaryMail) {
    console.log(`UserMail: ${userMail} & ReferrerMail: ${referrerMail} do not match!`);
    return;
  }
...
async function saveAttractionsDB() {  
    if (!localStorage['ak-userMail'] || !localStorage['ak-attractions-saved']) return;
...
!async function initMap() {
  ...
  if (localStorage['ak-attractions-saved']) {
    ...
    const morningTimeslotName = $morningTimeslot.getAttribute('data-ak-timeslot'); // .querySelector('[data-ak-timeslot-title]').textContent.trim().toLowerCase(); 

    const $afteTimeslotWrap  =$currentSlide.querySelector('[data-ak-timeslot-wrap="afternoon"]');
    const $afteTimeslot = $currentSlide.querySelector('[data-ak-timeslot="afternoon"]'); 
    const afteTimeslotName = $afteTimeslot.getAttribute('data-ak-timeslot'); // .querySelector('[data-ak-timeslot-title]').textContent.trim().toLowerCase(); 

    const $eveTimeslotWrap = $currentSlide.querySelector('[data-ak-timeslot-wrap="evening"]'); 
    const $eveTimeslot = $currentSlide.querySelector('[data-ak-timeslot="evening"]');
    const eveTimeslotName = $eveTimeslot.getAttribute('data-ak-timeslot'); // .querySelector('[data-ak-timeslot-title]').textContent.trim().toLowerCase(); 

    attractions.forEach((attraction, num) => {
      if (!localStorage['ak-userMail'] && addedAttractions >= 10) {
      ...
      if (coords && coords.trim() && Number(coords.trim().split(',')?.[0])) {
        ...
        if (coords.length) {
          ...
          if (attractions.length <= 3) {
            ...
            if (num === 0) {
              processAttractionToList($morningTimeslotWrap, morningTimeslotName, processObj);
            }
            else if (num === 1) {
              processAttractionToList($afteTimeslotWrap, afteTimeslotName, processObj); 
            }
            else if (num === 2) {
              processAttractionToList($eveTimeslotWrap, eveTimeslotName, processObj);
            }

            function processAttractionToList($timeslotWrap, timeslotName, processObj) {
              const { lat, lng, name, editorialSummary, slideIndex, marker } = processObj;
              addAttractionToList(name, $timeslotWrap, marker);
              const saveObj = {
                // marker,
                location: { lat, lng }, 
                displayName: name,
              ...
              if ($timeslotWrap === $morningTimeslotWrap) return;
              const $title = $timeslotWrap.closest('[data-ak-timeslots]').querySelector('[data-ak-timeslot-title]');
              $title.click(); 
            }
          }
          else {
  ...
  else if (localStorage['ak-referred']) {
    const userMail = localStorage['ak-userMail'];
    retrieveReferrerAttractions(userMail);
  }
}(); 

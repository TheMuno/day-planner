...
$calculatePasses.addEventListener('click', async e => {
  ...
  const userAddedAttractions = getAllSliderAttractionNames(); // JSON.parse(localStorage['user-added-items'] || '[]');
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    const isMatchedByName = Object.entries(userAddedAttractions).some(attraction => 

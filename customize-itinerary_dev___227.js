...
const $arrivalAirportWrap = document.querySelector('[data-ak-airport="arrival"]');
const $departureAirportWrap = document.querySelector('[data-ak-airport="departure"]');
...
window.addEventListener('load', async () => {
  ...
  if (Clerk.user) { // logged-in user retrieve from DB
    ...
    if (!data) {  // user has no saved record in the db
      ...
      const hotel = $hotelResultWrap.placeObj; // localStorage['ak-hotel'];

      const arrivalAirport = $arrivalAirportWrap.placeObj; // localStorage['ak-arrival-airport'];
      const departureAirport = $departureAirportWrap.placeObj; // localStorage['ak-departure-airport'];
    ...
    if (referrerMail) {
      ...
      if (localStorage['ak-update-travel-days']) {
        ...
        localStorage.removeItem('ak-update-travel-days');
      ...
      // userTravelDates = localStorage['ak-update-travel-days'] ? localStorage['ak-travel-days'] : travelDates;
      // localStorage.removeItem('ak-update-travel-days');
  ...
  function processSetupInfoData(travelDates, hotel, arrivalAirport, departureAirport, savedAttractions) {
    ...
    if (localStorage['ak-update-hotel']) {
      userHotel = $hotelResultWrap.placeObj; // localStorage['ak-hotel'];
    }
    else {
      userHotel = hotel;
    }
    ...
    console.log('$hotelResultWrap.placeObj', $hotelResultWrap.placeObj)

    if (localStorage['ak-update-arrival-airport']) {
      userArrivalAirport = $arrivalAirportWrap.placeObj; // localStorage['ak-arrival-airport'];
    }
    else {
      userArrivalAirport = arrivalAirport;
    }
    ...
    if (localStorage['ak-update-departure-airport']) {
      userDepartureAirport = $departureAirportWrap.placeObj; // localStorage['ak-departure-airport'];
    }
    else {
      userDepartureAirport = departureAirport;
    }
...
async function saveAttractionsDB() {  
    ...
    let hotel='', arrivalAirport='', departureAirport='', travelDates='', savedAttractions='{}';
    ...
    const $hotelWrap = document.querySelector('[data-ak="hotel-search-result"]');
    const $arrivalWrap = document.querySelector('[data-ak-airport="arrival"]');
    const $departureWrap = document.querySelector('[data-ak-airport="departure"]');

    hotel = $hotelWrap.placeObj;
    arrivalAirport = $arrivalWrap.placeObj;
    departureAirport = $departureWrap.placeObj;
...
!async function setupHotelAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    $hotelResultWrap.placeObj = JSON.stringify(placeObj);
  ...
  document.querySelectorAll('[data-ak="airport-autocomplete"]').forEach(autocomplete => {
    ...
    placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      if (airportType.includes('arrival')) {
        ...
        $arrivalAirportWrap.placeObj = JSON.stringify(placeObj);
        localStorage['ak-update-arrival-airport'] = true;
      }
      else {
        // localStorage['ak-departure-airport'] = JSON.stringify(placeObj);

        $departureAirportWrap.placeObj = JSON.stringify(placeObj);

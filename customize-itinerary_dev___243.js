...
document.querySelector('[travel-details]').addEventListener('click', e => {
  ...
  const dataAk = $attraction.getAttribute('data-ak');
  const airportType = $attraction.getAttribute('data-ak-airport');
  if (airportType) {
    if (airportType.includes('arrival')) {
      localStorage.removeItem('ak-arrival-airport');
    }

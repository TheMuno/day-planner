...
window.addEventListener('load', async () => {
  ...
  if (Clerk.user) {
    $saveItineraryBtn.closest('.ak-save-wrap').classList.remove('hidden');
  }
  else {
...
!async function initMap() {
  ...
  if (localStorage['ak-attractions-saved']) {
    ...
    const afteTimeslotName = $afteTimeslot.querySelector('[data-ak-timeslot-title]').textContent.trim().toLowerCase(); 
    ...
    const eveTimeslotName = $eveTimeslot.querySelector('[data-ak-timeslot-title]').textContent.trim().toLowerCase(); 

    attractions.forEach((attraction, num) => {
      if (!Clerk.user && addedAttractions >= 10) {
...
if (localStorage['ak-travel-days']) {
  ...
  function reInitWebflow() {
    ...
    console.log('Reinit Webflow ran!!')
  }
} 

async function retrieveSavedAttractions() {
  const userMail = localStorage['ak-userMail'];
  if (!userMail) return;
  const userRef = doc(db, 'locationsData', `user-${userMail}`); 
  const docSnap = await getDoc(userRef);
  if (!docSnap.exists()) {
      console.log(`User: ${userMail} does not exist!`);
      return;
  }
  const data = await docSnap.data(); 

  const { savedAttractions } = data; 
  processSavedAtractions(savedAttractions); 
}
...
function processSavedAtractions(savedAttractions) {
  for (const [slide, attractions] of Object.entries(savedAttractions)) {
    const slideNum = Number(slide.match(/\d+/)[0]);
    const $currentSlide = [...$attractionsSliderMask.querySelectorAll('.w-slide')][slideNum-1];
    const $morningWrap = $currentSlide.querySelector('[data-ak-timeslot-wrap="morning"]');

...
!async function setupHotelAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    addLocationToResultWrap(displayName, marker, $hotelWrap); 
  });

  function addLocationToResultWrap(name, marker, $hotelWrap) {
...
!async function setupAirportAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    const $airportWrap = document.querySelector('[data-ak="airport-search-result"]');
    addHotel(displayName, marker, $airportWrap); 
  });

  function addHotel(name, marker, $airportWrap) {
    const $location = document.querySelector('[data-ak="attraction-location"]');
    $location.classList.remove('hidden');
    $location.querySelector('[data-ak="location-title"]').textContent = name; 
    $location.querySelector('[data-ak="location-link-text"]').textContent = name;
    $location.marker = marker;
    // $location.saveObj = saveObj;
    $airportWrap.append($location);
  }
}(); 

...
async function showNewPlacesLayer(type, markerArray) {
  ...
  currentBounds = new google.maps.LatLngBounds(currentBounds);

  // Define the request using the map's current view
  const request = {
    includedPrimaryTypes: type, // e.g., ['restaurant']
    locationRestriction: currentBounds, // locationRestriction: map.getBounds(), // Search only what's visible
    maxResultCount: 20,

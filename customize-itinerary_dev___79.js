...
const $unsavedChanges = document.querySelector('.ak-changes-made-text');
const markerArr = [];
...
window.addEventListener('load', async () => {
  ...
  if (Clerk.user) { // logged-in user retrieve from DB
    ...
    const userMail = localStorage['ak-userMail']; // set in site settings once user logs-in
    ...
    if (referrerMail) {
      ...
      setupUserInfo(savedAttractions, tripName, travelDates);
    ...
    setupUserInfo(savedAttractions);
  }
  else {
    showTripInfoHeader(); 
  }
...
function setupUserInfo(savedAttractions, tripName=undefined, travelDates=undefined) {
  processTripInfoHeader(tripName, travelDates);
  processSavedAttractions(savedAttractions);
}

async function processTripInfoHeader(tripName, travelDates) {
  await setupTripNameNTravelDates(tripName, travelDates); 
  showTripInfoHeader(); 
}
...
async function setupTripNameNTravelDates(tripName, travelDates) {
  if (tripName) {
  ...
  if (travelDates) {
    ...
    await setupTravelDates(flatpickrDate); 
...
async function setupTravelDates(flatpickrDate) {
  // const { flatpickrDate } = JSON.parse(localStorage['ak-travel-days']);
  let [ startDate, endDate ] = flatpickrDate.split(/\s+to\s+/);
  // startDate = getLocalTimezoneDate(startDate);
  // endDate = getLocalTimezoneDate(endDate);

  console.log('flatpickrDate', flatpickrDate)
  console.log('flatpickrDate.split(/\s+to\s+/)', flatpickrDate.split(/\s+to\s+/))
  ...
  function setupSliderDates() {
    ...
    console.log('theStartDate', theStartDate)
    ...
    for (let i = 0; i < numberOfDays; i++) {
      ...
      console.log(`newDate${i+1}`, newDate)

      $attractionsSliderMask.append($slideClone);
    } 

    console.log('$attractionsSliderMask 2::::::', $attractionsSliderMask)
  }
  ...
  function getLocalTimezoneDate(dateStr) {
    const [ year, month, day ] = dateStr.split('-');
    const reorderedDate = `${month}-${day}-${year}`;
    const localTimezoneDate = new Date(reorderedDate);
...
function restoreSavedAttractions(savedAttractions) {
  ...
  console.log('$attractionsSlider', $attractionsSlider)
  console.log('$attractionsSliderMask', $attractionsSliderMask)

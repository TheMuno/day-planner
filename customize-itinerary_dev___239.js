...
const $arrivalAirportResultWrap = document.querySelector('[data-ak="airport-search-result"][data-ak-airport="arrival"]');
const $departureAirportResultWrap = document.querySelector('[data-ak="airport-search-result"][data-ak-airport="departure"]');
...
window.addEventListener('load', async () => {
  ...
  if (Clerk.user) { // logged-in user retrieve from DB
    ...
    if (!data) {  // user has no saved record in the db
      ...
      const hotel = $hotelResultWrap.saveObj; // localStorage['ak-hotel'];
      const arrivalAirport = $arrivalAirportResultWrap.saveObj; // localStorage['ak-arrival-airport'];
      const departureAirport = $departureAirportResultWrap.saveObj; // localStorage['ak-departure-airport'];
  ...
  function processSetupInfoData(travelDates, hotel, arrivalAirport, departureAirport, savedAttractions) {
    ...
    if (localStorage['ak-update-hotel']) {
      userHotel = $hotelResultWrap.saveObj // localStorage['ak-hotel'];
    }
    else {
      userHotel = hotel;
    }
    ...
    if (localStorage['ak-update-arrival-airport']) {
      userArrivalAirport = $arrivalAirportResultWrap.saveObj // localStorage['ak-arrival-airport'];
    }
    else {
    ...
    if (localStorage['ak-update-departure-airport']) {
      userDepartureAirport = $departureAirportResultWrap.saveObj // localStorage['ak-departure-airport'];
    }
    else {
...
async function saveAttractionsDB() {  
    ...
    hotel = $hotelResultWrap.saveObj;
    arrivalAirport = $arrivalAirportResultWrap.saveObj;
    departureAirport = $departureAirportResultWrap.saveObj;
...
!async function setupHotelAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    $hotelResultWrap.saveObj = JSON.stringify(placeObj);
  ...
  document.querySelectorAll('[data-ak="airport-autocomplete"]').forEach(autocomplete => {
    ...
    placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      if (airportType.includes('arrival')) {
        $arrivalAirportResultWrap.saveObj = JSON.stringify(placeObj);
        // localStorage['ak-arrival-airport'] = JSON.stringify(placeObj);
        localStorage['ak-update-arrival-airport'] = true;
      }
      else {
        $departureAirportResultWrap.saveObj = JSON.stringify(placeObj);

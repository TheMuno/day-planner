...
async function retrieveReferrerAttractions(userMail) {
  ...
  const referrerData = await referrerSnap.data(); 
  let { secondaryMail, savedAttractions } = referrerData;
  ...
  console.log('Mail Email:', userMail)
...
if (localStorage['ak-travel-days']) {
  ...
  function setupTitleTravelDates() {
    ...
    const titleDates = (titleStartDate === titleEndDate) ? titleStartDate : `${titleStartDate} - ${titleEndDate}`;
...
function restoreSavedAttractions(savedAttractions) {
  ...
  for (const [slide, attractions] of Object.entries(savedAttractions)) {
    ...
    if (morning) {
      processTimeslot(morning, $morningWrap, slideNum);
    }
    if (afternoon) {
      processTimeslot(afternoon, $afternoonWrap, slideNum);
    }
    if (evening) {
      processTimeslot(evening, $eveningWrap, slideNum);
    }
  }

  function processTimeslot(timeslot, $sectionWrap, slideNum) {

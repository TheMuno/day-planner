...
const $onPassTicketsCounter = document.querySelector('[data-ak="on-pass-tickets"]');
...
$calculatePasses.addEventListener('click', async e => {
  ...
  const placeIds = JSON.parse(localStorage['ak-place-ids'] || '[]'); 
  const userAddedAttractions = Object.entries(getAllSliderAttractionNames()); // JSON.parse(localStorage['ak-user-added-items'] || '[]'); 
  ...
  $ticketsNum.forEach(n => n.textContent = attractionsNum);
  ...
  function addattractionToPassList(passes, passName, $resultEl, place_id, $passContainer) {
    ...
    return activePass ? name : undefined;
  }
...
function getAllSliderAttractionNames() {
  ...
  localStorage['ak-user-added-items'] = savedAttractions; 
  return savedAttractions;
}

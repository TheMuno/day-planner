...
async function renderData() {
  ...
  try {
    ...
    preliminaryStr += `${processTitleDates(travelDates)}\n\n`;
    
    if (hotel) {
      hotelName = JSON.parse(hotel).displayName;
      preliminaryStr += `Hotel\n${hotelName}\n\n`;
    }
    if (arrivalAirport) {
      arrival = JSON.parse(arrivalAirport).displayName;
      preliminaryStr += `Arrival Location\n${arrival}\n\n`;
    }
    if (departureAirport) {
      departure = JSON.parse(departureAirport).displayName;
      preliminaryStr += `Departure Location\n${departure}\n\n`;
    }
            
    attractionLocations = JSON.parse(savedAttractions);
  } catch (err) {
...
function processTitleDates(date) {
  ...
  const [ startDate, endDate ] = dateStr.split(/\s+to\s+/);

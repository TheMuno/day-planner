...
window.addEventListener('load', async () => {
  ...
  let addedAttractions = Number(localStorage['ak-addedAttractions-count'] || 0);
  ...
  function handleDrop(e) {
    ...
    const { attractionName, fromTimeslot } = parseJSON(data);
    if (!attractionName || !fromTimeslot) {
      console.log('Missing attractionName or fromTimeslot');
      return;
    }
    ...
    // else {
      ...
      const savedAttractions = localStorage['ak-attractions-saved'] ? parseJSON(localStorage['ak-attractions-saved']) : {};
...
function processSavedAttractions(savedAttractions) {
  if (savedAttractions) {
    const savedAttractionsParsed = parseJSON(savedAttractions);
    if (!savedAttractionsParsed) {
      console.log('Missing valid savedAttractions\nData possibly corrupted!');
      return;
    }
    localStorage.removeItem('ak-place-ids'); // reset placeIds before restoration
...
function setupTripNameNTravelDates(tripName, travelDates) {
  ...
  if (travelDates) {
    const { flatpickrDate } = parseJSON(travelDates);
...
function setupHotelNAirports(hotel, arrivalAirport, departureAirport) {
  ...
  function processLocation(location, $resultWrapName) {
    const locationDetails = parseJSON(location);
    if (!locationDetails) {
      console.log('Missing locationDetails');
      return; 
    }
  ...
  function setupLocation(location, locationDetails, $resultWrap) {
    ...
    /*if (location === hotel) {
      ...
      if (location === arrivalAirport) {
        if (markerObj['airport-arrival']) {
          markerObj['airport-arrival'].setMap(null);
        }
        markerObj['airport-arrival'] = marker;
      }
      else {
        if (markerObj['airport-departure']) {
          markerObj['airport-departure'].setMap(null);
        }
  ...
  $attractionsSlider.querySelectorAll('.w-slide').forEach((slide, n) => {
    ...
    slide.querySelectorAll('[data-ak-timeslots] [data-ak-timeslot-content]').forEach(timeslotContent => {
      ...
      timeslotContent.querySelectorAll('[data-ak="attraction-location"]:not(.hidden)').forEach(attraction => {
        ...
        const { saveObj: { location, displayName, editorialSummary, placeId } } = attraction;          
...
function addAttractionToList(name, $listName, marker=null, saveObj={}) {
  ...
  const { placeId } = saveObj; // what if $location added on multiple slides / slide-sections
  if (placeId) {
...
!async function setupAirportAutocomplete() {
  ...
  document.querySelectorAll('[data-ak="airport-autocomplete"]').forEach(autocomplete => {
    ...
    placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      console.log('airportType', airportType)
      console.log("airportType.includes('arrival')", airportType.includes('arrival'))

      if (airportType.includes('arrival')) {
        if (markerObj['airport-arrival']) {
          markerObj['airport-arrival'].setMap(null);
        }
        // marker = createMarker(displayName, {lat, lng}, editorialSummary, airportMarkerPinUrl); 
...
$calculatePasses.addEventListener('click', async e => {
  ...
  const attractions = parseJSON(localStorage['ak-sheet-attractions'] || '{}');
  if (!attractions) {
  ...
  const placeIds = parseJSON(localStorage['ak-place-ids'] || '[]'); 
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    addattractionToPassList(passes, 'go city', $result, $gocityResultsContainer);
    addattractionToPassList(passes, 'citypass', $result, $citypassResultsContainer); 
  }
  ...
  function addattractionToPassList(passes, passName, $result, $passContainer) {
  ...
  populatePasses(passData, 'gocity_explorer', $gocityName, $gocityPrice, $gocityResultsContainer); 
  populatePasses(passData, 'citypass', $citypassName, $citypassPrice, $citypassResultsContainer); 

  function populatePasses(passData, passName, $passNameEl, $passPriceEl, $passContainer) {
    // console.log('populatePasses function')
    // console.log('passName:', passName)
    resetExtraPass($passNameEl); 
    // console.log('resetExtraPass done!!') 

    const passAttractionsNum = $passContainer.querySelectorAll('.active').length;
    // console.log('passAttractionsNum:', passAttractionsNum) 
...
function handleSliderRemoveLocation(e) {
  ...
  // else {
    ...
    if (savedAttractions && Object.keys(savedAttractions).length) {
      ...
      const savedAttractionsParsed = parseJSON(savedAttractions);
      if (!savedAttractionsParsed) {
        console.log('Missing savedAttractions');
        return;
      }
  ...
  if ($attraction.placeId) {
    // console.log('placeId on removal:', $attraction.placeId)
    const placeIds = JSON.parse(localStorage['ak-place-ids'] || '[]');
...
/*if (!localStorage['ak-referred']) {
  ...
  if (localStorage['ak-travel-days']) {
    const { flatpickrDate } = parseJSON(localStorage['ak-travel-days']);
    setupTravelDates(flatpickrDate); 
  }
}*/
...
function restoreSavedAttractions(savedAttractions) {
  ...
  function processSectionAttractions(attractions, $sectionWrap, slideNum) {
    attractions.forEach(attraction => {
      const { displayName, editorialSummary, location, type, placeId } = attraction;
      const marker = createMarker(displayName, location, editorialSummary, type);
      const saveObj = { displayName, editorialSummary, location, placeId };
  ...
  $attractionsSlider.querySelectorAll('.w-slide').forEach((slide, n) => {
    slide.querySelectorAll('[data-ak-timeslots] [data-ak-timeslot-content]').forEach(timeslotContent => {
      timeslotContent.querySelectorAll('[data-ak="attraction-location"]:not(.hidden)').forEach(attraction => {
        ...
        savedAttractions[normalize(name)] = placeId;
      });
    });
  });
  return savedAttractions;

...
// const $unsavedDateChanges = document.querySelector('[data-ak="date-changes"]');
const markerArr = [];
...
window.addEventListener('load', async () => {
  ...
  $unsavedDateChanges.addEventListener('click', async e => {
    console.log('clicked!!')
    const alertText = $unsavedDateChanges.textContent;
    $unsavedDateChanges.textContent = 'saving...';
    await saveAttractionsDB();
    console.log('============================');
    console.log('Saved to DB')
    localStorage.removeItem('ak-attractions-saved');
    $unsavedDateChanges.classList.add('hide');    
    $unsavedDateChanges.textContent = alertText;
  });*/
  ...
  if (Clerk.user) { // logged-in user retrieve from DB
    ...
    if (referrerMail) {
      ...
      const { secondaryMail, tripName, travelDates, savedAttractions, ModifiedAt } = referrerData;
      ...
      setupUserInfo(savedAttractions, tripName, userTravelDates);
    ...
    setupUserInfo(savedAttractions, undefined, travelDates);
  }
  else {
...
function setupUserInfo(savedAttractions=undefined, tripName=undefined, travelDates=undefined) {
...
function setupHotelNAirports() {
  if (localStorage['ak-hotel']) {
    processLocation('ak-hotel', '[data-ak="hotel-search-result"]');
  }

  if (localStorage['ak-arrival-airport']) {
    processLocation('ak-arrival-airport', '[data-ak-airport="arrival"]'); 
  }

  if (localStorage['ak-departure-airport']) {
    processLocation('ak-departure-airport', '[data-ak-airport="departure"]'); 
  }

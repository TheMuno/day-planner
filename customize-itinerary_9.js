...
document.body.addEventListener('click', e => {
  ...
  console.log('addedAttractions before subtract::', addedAttractions)
  addedAttractions -= 1;

  console.log('addedAttractions after subtract::', addedAttractions)
});
...
if (localStorage['ak-travel-days']) {
  ...
  function setupTitleTravelDates() {
    let titleStartDate = getUTC(startDate);
    let titleEndDate = getUTC(endDate);
  ...
  function setupSliderDates() {
    ...
    const startDateUTC = getUTC(startDate);
    updateDayNDate($firstSlide, getDateDetails(startDateUTC)); 
    
    for (let i = 0; i < numberOfDays; i++) {
      const newDate = new Date(startDateUTC.setDate(startDateUTC.getDate() + 1)); 

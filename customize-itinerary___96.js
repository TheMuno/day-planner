...
window.addEventListener('load', async () => {
  ...
  if (localStorage['ak-referred']) {
    ...
    if (Clerk.user) { // logged-in user retrieve from DB
      ...
      if (!data) return;
      let { tripName, travelDates, savedAttractions } = data;
...
function restoreSavedAttractions(savedAttractions) {
  ...
  for (const [slide, attractions] of Object.entries(savedAttractions)) {
    ...
    console.log('savedAttractions', savedAttractions)
    console.log("[...$attractionsSliderMask.querySelectorAll('.w-slide')]", [...$attractionsSliderMask.querySelectorAll('.w-slide')])
    console.log("[...$attractionsSliderMask.querySelectorAll('.w-slide')][slideNum-1]", [...$attractionsSliderMask.querySelectorAll('.w-slide')][slideNum-1])

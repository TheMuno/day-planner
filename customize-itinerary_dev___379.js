...
async function initMap(center) {
  ...
  document.getElementById('showDiningButton').addEventListener('click', () => {
...
async function toggleNewPlacesLayer(type, markerArray, buttonId) {
  ...
  if (markerArray.length > 0) {
    ...
    await showNewPlacesLayer(type, markerArray);
...
function createPlaceMarker(place) {
  ...
  markerIcon.className = 'custom-dining-marker';
  markerIcon.textContent = '🍽️';
...
const $toggleTransitWrap = document.querySelector('.ak-toggle-transit-wrap');
$toggleTransitWrap.addEventListener('click', async e => {
  if (!e.target.classList.contains('ak-toggle-transit')) return;

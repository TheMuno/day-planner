...
let placeIds = localStorage['ak-place-ids'] ? JSON.parse(localStorage['ak-place-ids']) : [];
...
$calculatePasses.addEventListener('click', async e => {
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    const { place_id, on_pass } = passInfo;
  ...
  populatePasses(passData); 

  function populatePasses(passData) {
    const gocityMatches = passData.filter(([id, data]) => data.pass_id.includes('gocity_explorer'));
    const gocitySortedPasses = gocityMatches
      .sort((a, b) => Number(a[1].attraction_count) - Number(b[1].attraction_count));

    const gocityAttractionsNum = $gocityResultsContainer.children.length;

    const gocityExactPass = gocitySortedPasses.find(([id, pass]) => {
      // console.log('pass.attraction_count:', pass.attraction_count)
      return Number(pass.attraction_count) === gocityAttractionsNum
    });

    console.log('$gocityResultsContainer.children.length:', $gocityResultsContainer.children.length)
    console.log('gocityExactPass:', gocityExactPass)
    
    if (gocityExactPass) {
      $gocityName.textContent = gocityExactPass[1].pass_name;
      $gocityPrice.textContent = gocityExactPass[1].pass_price; 
    }
    else {
      const passUpperLimit = gocitySortedPasses.find(([id, pass]) => pass.attraction_count >= gocityAttractionsNum);
      const passLowerLimit = gocitySortedPasses.reverse().find(([id, pass]) => pass.attraction_count <= gocityAttractionsNum);
      ...
      showExtraPass($gocityName, passUpperLimit);
    }

    $gocityName.closest('[data-ak="pass-info"]').classList.remove('hidden');

    const citypassMatches = passData.filter(([id, data]) => data.pass_id.includes('citypass'));
    const citypassSortedPasses = citypassMatches
      .sort((a, b) => Number(a[1].attraction_count) - Number(b[1].attraction_count));

    const citypassAttractionsNum = $citypassResultsContainer.children.length;

    const citypassExactPass = citypassSortedPasses.find(([id, pass]) => {
      // console.log('pass.attraction_count:', pass.attraction_count)
      return Number(pass.attraction_count) === citypassAttractionsNum
    });

    console.log('$citypassResultsContainer.children.length:', $citypassResultsContainer.children.length)
    console.log('citypassExactPass:', citypassExactPass)

    if (citypassExactPass) {
      $citypassName.textContent = localStorage['ak-pass-city-pass-name'];
      $citypassPrice.textContent = `$${localStorage['ak-pass-city-pass-price']}`;
    }
    else {
      const passUpperLimit = citypassSortedPasses.find(([id, pass]) => pass.attraction_count >= citypassAttractionsNum);
      const passLowerLimit = citypassSortedPasses.reverse().find(([id, pass]) => pass.attraction_count <= citypassAttractionsNum);

      console.log('passUpperLimit:', passUpperLimit)
      console.log('passLowerLimit:', passLowerLimit)

      if (passLowerLimit && passLowerLimit.length) {
        $citypassName.textContent = passLowerLimit[1].pass_name;
        $citypassPrice.textContent = passLowerLimit[1].pass_price;
      }
      else {
        $citypassName.closest('[data-ak="pass-info"]').textContent = 'No lower pass';
        $citypassPrice.textContent = '';
      }
      showExtraPass($citypassName, passUpperLimit);
    }
    
    $citypassName.closest('[data-ak="pass-info"]').classList.remove('hidden');

    function showExtraPass($passName, passUpperLimit) {
      ...
      // console.log('$passInfoClone.querySelector(`[data-ak="pass-name"]`)', $passInfoClone.querySelector(`[data-ak="pass-name"]`))

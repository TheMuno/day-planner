...
function setupHotelNAirports(hotel, arrivalAirport, departureAirport) {
  ...
  function setupLocation(location, locationDetails, $resultWrap) {
    ...
    if (location === hotel) {
      ...
      marker = createMarker(displayName, {lat, lng}, editorialSummary, hotelMarkerPinUrl); 
...
function createMarker(title, position, editorialSummary=title, markerPinSrc=markerPinUrl) {
  const markerPinImg = document.createElement('img');
  markerPinImg.src = markerPinSrc;
...
!async function setupHotelAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    const marker = createMarker(displayName, {lat, lng}, editorialSummary, hotelMarkerPinUrl); 

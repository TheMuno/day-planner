...
$calculatePasses.addEventListener('click', async e => {
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    if (passes.toLowerCase().includes('go city')) {
      ...
      $ticketName.innerHTML = `<s class="strike">${name}</s>`; 
  ...
  function populatePasses(passData) {
    ...
    const gocityAttractionsNum = $gocityResultsContainer.querySelectorAll('.active').length;
    ...
    if (gocityExactPass) {
      ...
      showExtraPass($gocityName, $gocityPrice, passUpperLimit, lowerLimitExists);
    }
    ...
    const citypassAttractionsNum = $citypassResultsContainer.querySelectorAll('.active').length;
    ...
    if (citypassExactPass) {
      ...
      showExtraPass($citypassName, $citypassPrice, passUpperLimit, lowerLimitExists);
    }

...
window.addEventListener('load', async () => {
  ...
  // end of logged-in user code
    ...
    localStorage['ak-update-attractions-unlogged-in'] = true;
  }
  else {
    const travelDates = localStorage['ak-travel-days'];
  ...
  function processSetupInfoData(travelDates, hotel, arrivalAirport, departureAirport, savedAttractions) {
    ...
    if (localStorage['ak-update-attractions']) {
      if (localStorage['ak-update-attractions-unlogged-in']) {
        mergelocalNDBAttractions(); 
      }
  ...
  function mergelocalNDBAttractions() {
    ...
    const savedAttractions = JSON.parse(localStorage['sample-saved']);
    for (const [slide, attractions] of Object.entries(savedAttractions)) {
        ...
        const savedMorningArr = savedAttractions[slide].morning;
        const savedAfternoonArr = savedAttractions[slide].afternoon;
        const savedEveningArr = savedAttractions[slide].evening;
    ...
    localStorage.removeItem('ak-update-unlogged');
  }

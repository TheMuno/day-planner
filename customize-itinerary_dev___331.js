...
document.querySelector('[data-ak="generate-text-file"]').addEventListener('click', e => {
  ...
  for (const [day, attractions] of Object.entries(userAttractions)) {
    ...
    for (const [section, sectionAttractions] of Object.entries(attractions)) {
      ...
      console.log('sectionAttractions', sectionAttractions)
      if (!sectionAttractions.length) continue;
      dayStr += `${sectionAttractions[0]}: ${sectionAttractions[1]}\n`;
      console.log('dayStr', dayStr)
    }
    /*dayStr += attractions.map(attraction => {
      if (!attraction || !attraction.length) return;
      return `${attraction[0]}: ${attraction[1]}`;
    });*/
  }
  console.log('Final dayStr::', dayStr)
  // return dayStr;
});

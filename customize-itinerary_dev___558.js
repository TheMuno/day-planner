...
$calculatePasses.addEventListener('click', async e => {
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    /*$individualResultsContainer.addEventListener('click', e => {
      if (!e.target.closest('[data-ak="ticket-buy-btn"]')) return;
      const $buyBtn = e.target.closest('[data-ak="ticket-buy-btn"]');
      const link = $buyBtn.getAttribute('buy-link');
      window.open(link);
    });*/

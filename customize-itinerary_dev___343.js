...
async function fetchItinerary() {
  const getItinerary = httpsCallable(functions, "getItinerary");
  const result = await getItinerary();
  console.log("Itinerary:", result.data);
  console.log("Itinerary:", result.data.locationsData);
}
fetchItinerary();
  ...
  function hideShowLoginNSavebtn() {
    ...
    if (Clerk.user) {
      ...
      $loginBtn.addEventListener('click', e => {
        ...
        if (window.location.pathname === undefined) { // Safari iOs
          Clerk.openSignUp();
        }
        else {
          Clerk.openSignUp({
...
function setupHotelNAirports(hotel, arrivalAirport, departureAirport) {
  ...
  function setupLocation(location, locationDetails, $resultWrap) {
    ...
    if (location === hotel) {
      ...
      marker = createMarker(displayName, {lat, lng}, editorialSummary, type, airportMarkerPinUrl); 
...
async function saveAttractionsDB() {  
    ...
    if (userSnap.exists()) {
      ...
      ModifiedAt = serverTimestamp();
      saveObj.ModifiedAt = ModifiedAt;
      await updateDoc(userRef, saveObj);
    }
    else {
      // console.log('userSnap does not exist')
      CreatedAt = serverTimestamp();
      saveObj.CreatedAt = CreatedAt;
      await setDoc(userRef, saveObj);
    }
    ...
    submitUserDetailsToWebflowForm(CreatedAt, ModifiedAt, hotel, arrivalAirport, departureAirport, tripName, travelDates, savedAttractions);
}
...
document.querySelector('[data-ak="generate-text-file"]').addEventListener('click', e => {
  let dayStr = '';
  const userAttractions = createATextFileForLocations();
  for (const [day, attractions] of Object.entries(userAttractions)) {
    dayStr += `${dayStr.length ? '\n\n\n' : ''}${day}\n`;   
    for (const [section, sectionAttractions] of Object.entries(attractions)) {   
      dayStr += `\n${section}\n\n`;
      if (!sectionAttractions.length) {
        dayStr += '...\n';
        continue;
      }
      sectionAttractions.forEach(a => {
        dayStr += `${a[0]}: ${a[1]}\n`;
      });      
    }    
  }
  // console.log('Final dayStr::\n\n', dayStr)
  download(dayStr);
});

function download(text, filename='User Attractions') {
...
function createATextFileForLocations() {
  const dayItinerary = {}; 
  const $attractionsSlider = document.querySelector('[data-ak="locations-slider"]');
  $attractionsSlider.querySelectorAll('.w-slide').forEach((slide, n) => {
    const day = slide.querySelector('[data-ak="timeslots-day"]').textContent.trim();
    const date = slide.querySelector('[data-ak="timeslots-date"]').textContent.trim();

    const dayTitle = `${day} ${date}`;
    dayItinerary[dayTitle] = {};

    const $sections = slide.querySelectorAll('[data-ak-timeslots="true"]');
    $sections.forEach(section => {
      const attractions = [];
      const title = section.querySelector('[data-ak-timeslot-title]').textContent.trim();
      const $attractionLocations = [...section.querySelectorAll('[data-ak-timeslot-content] [data-ak="attraction-location"]:not(.hidden)')];
      $attractionLocations.forEach(loc => {
        const locName = loc.querySelector('[data-ak="location-title"]').textContent.trim();
        const locLat = loc.marker?.position.lat || null;
        const locLng = loc.marker?.position.lng || null;
        let locLink = '';
        if (locLat) {
          locLink = `${directionsUrlBase}?api=1&query=${locLat}%2C${locLng}`;  
        }
        attractions.push([locName, locLink]);
      });
      dayItinerary[dayTitle][title] = attractions;
    });
  });
  return dayItinerary;
}
...
function submitUserDetailsToWebflowForm(CreatedAt, ModifiedAt, hotel, arrivalAirport, departureAirport, tripName, travelDates, savedAttractions) {
  const $submitUserDetailsForm = document.querySelector('.ak-submit-user-trip-details');
  const $createAtSubmit = $submitUserDetailsForm.querySelector('[data-ak="created-at-submit"]');
  const $modifiedAtSubmit = $submitUserDetailsForm.querySelector('[data-ak="modified-at-submit"]');
  const $hotelSubmit = $submitUserDetailsForm.querySelector('[data-ak="hotel-submit"]');
  const $arrivalAirportSubmit = $submitUserDetailsForm.querySelector('[data-ak="arrival-airport-submit"]');
  const $departureAirportSubmit = $submitUserDetailsForm.querySelector('[data-ak="departure-airport-submit"]');
  const $tripNameSubmit = $submitUserDetailsForm.querySelector('[data-ak="trip-name-submit"]');
  const $travelDatesSubmit = $submitUserDetailsForm.querySelector('[data-ak="travel-dates-submit"]');
  const $savedAttractionsSubmit = $submitUserDetailsForm.querySelector('[data-ak="saved-attractions-submit"]');
  const $submitUserDetailsBtn = $submitUserDetailsForm.querySelector('.ak-submit-user-details-btn');

  $createAtSubmit = CreatedAt ? CreatedAt : '';
  $modifiedAtSubmit = ModifiedAt ? ModifiedAt : '';
  $hotelSubmit.value = hotel ? hotel : '';
  $arrivalAirportSubmit = arrivalAirport ? arrivalAirport : '';
  $departureAirportSubmit = departureAirport ? departureAirport : '';
  $tripNameSubmit = tripName ? tripName : '';
  $travelDatesSubmit = travelDates ? travelDates : '';
  $savedAttractionsSubmit = savedAttractions ? savedAttractions : '';

  $submitUserDetailsBtn.click();
}
...
!async function setupAirportAutocomplete() {
  ...
  document.querySelectorAll('[data-ak="airport-autocomplete"]').forEach(autocomplete => {
    ...
    placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      const marker = createMarker(displayName, {lat, lng}, editorialSummary, type, airportMarkerPinUrl); 
      // markerArr.push(marker);

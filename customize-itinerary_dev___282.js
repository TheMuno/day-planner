...
// const markerArr = [];
const markerObj = {};
console.log('markerObj 1::', markerObj)
...
function setupHotelNAirports(hotel, arrivalAirport, departureAirport) {
  ...
  function setupLocation(location, locationDetails, $resultWrap) {
    ...
    console.log('markerObj 2::', markerObj)
...
!async function setupHotelAutocomplete() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
    ...
    // markerArr.push(marker); 
    markerObj['hotel'] = marker;
    console.log('markerObj 3::', markerObj)
...
!async function setupAirportAutocomplete() {
  ...
  document.querySelectorAll('[data-ak="airport-autocomplete"]').forEach(autocomplete => {
    ...
    placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      console.log('markerObj 4::', markerObj)
...
!async function setupAutocompleteInp() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      console.log('markerobj at first::', markerObj)
      const marker = createMarker(displayName, {lat, lng}, editorialSummary); 
      // markerArr.push(marker); 
      // const currentSlideMarkersArr = markerObj[`slide${slideIndex}`] || [];
      // currentSlideMarkersArr.push(marker);

      // let currentSlideMarkersArr = markerObj[`slide${slideIndex}`];
      markerObj[`slide${slideIndex}`] = markerObj[`slide${slideIndex}`] || [];
      markerObj[`slide${slideIndex}`].push(marker);

      console.log('markerObj 5::', markerObj)
      // console.log('marker', marker)
      // console.log('currentSlideMarkersArr', currentSlideMarkersArr)
...
function handleTravelResultsRemoveLocation(e) {
  ...
  console.log('markerObj 6::', markerObj)
...
function handleSliderRemoveLocation(e) {
  ...
  if ($attraction.marker) {
    ...
    const currentSlideMarkersArr = markerObj[`slide${slideIndex}`] || [];
    currentSlideMarkersArr.splice(currentSlideMarkersArr.indexOf($attraction.marker), 1); 

    console.log('markerObj 7::', markerObj)
  }
...
console.log('map', map)
console.log('markerObj 8::', markerObj)

document.querySelectorAll('.ak-map-pins-wrap input[type=radio]').forEach(radioBtn => {
...
document.querySelectorAll('.w-slider-arrow-left, .w-slider-arrow-right').forEach(sliderArrow => {

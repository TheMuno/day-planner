...
$calculatePasses.addEventListener('click', async e => {
  ...
  const addedAttractionsSet = new Set([undefined]);
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    const adddedGoPass = addattractionToPassList(passes, 'go city', $result, place_id, $gocityResultsContainer);
    const addedCityPass = addattractionToPassList(passes, 'citypass', $result, place_id, $citypassResultsContainer); 

    addedAttractionsSet.add(adddedGoPass);
    addedAttractionsSet.add(addedCityPass);
  }
  ...
  $onPassTicketsCounter.textContent = addedAttractionsSet.size - 1; // set created with 1 element (undefined) 

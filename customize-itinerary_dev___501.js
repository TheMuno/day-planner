...
window.addEventListener('load', async () => {
  ...
  function mergelocalNDBAttractions(savedAttractionsDB) {
    ...
    for (const [slide, attractions] of Object.entries(savedAttractions)) {
        ...
        savedAttractions[slide].morning = [...new Set([...savedMorningArr, ...localMorningArr])];
        savedAttractions[slide].afternoon = [...new Set([...savedAfternoonArr, ...localAfternoonArr])];
        savedAttractions[slide].evening = [...new Set([...savedEveningArr, ...localEveningArr])];
    }
...
$calculatePasses.addEventListener('click', async e => {
  ...
  for (const [id, passInfo] of Object.entries(attractions)) {
    ...
    const isMatchedByName = userAddedAttractions.some(attraction => 
        attraction[0].includes(normalizedAttractionName)
        // !!isLikelyMatch(attraction[0], normalizedAttractionName)
    );

    if ((!isMatchedById && !isMatchedByName) || on_pass.trim().toLowerCase() !== 'true') {
      continue;
    }
...
!async function setupAutocompleteInp() {
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
      if (!Clerk.user) {
        ...
        // console.log('addedAttractions before...', addedAttractions)
        addedAttractions += 1;
        localStorage['ak-addedAttractions-count'] = addedAttractions; 
        // console.log('addedAttractions after...', addedAttractions)

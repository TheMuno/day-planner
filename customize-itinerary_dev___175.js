...
!async function setupAutocompleteInp() {
  ...
  console.log('placeAutocomplete.target?.Zg', placeAutocomplete.target?.Zg)
  ...
  if (placeAutocomplete.target?.Zg) {
    placeAutocomplete.target?.Zg.setAttribute('placeholder', searchAttractionsPlaceholderTxt);
  }
  ...
  placeAutocomplete.addEventListener('gmp-select', async (res) => {
      ...
      console.log('res', res)
      console.log('res.srcElement', res.srcElement)
      console.log('res.target.Zg', res.target.Zg)
      ...
      function resetUserInputField() {
        if (!res.srcElement?.Eg) return;
        const $userInput = res.target?.Zg; // res.srcElement?.Eg;
        if (!$userInput) return;
        $userInput.value = '';

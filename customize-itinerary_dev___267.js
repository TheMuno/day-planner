...
window.addEventListener('load', async () => {
  ...
  function mergelocalNDBAttractions(savedAttractions) {
    const slide1MorningArr = [],
        slide2MorningArr = [],
        slide1AfternoonArr = [],
        slide2AfternoonArr = [],
        slide1EveningArr = [],
    ...
    // console.log('savedAttractionsDB', savedAttractionsDB)
    // const savedAttractions = { ...savedAttractionsDB }; 
    // console.log('savedAttractions 1::', savedAttractions)
    for (const [slide, attractions] of Object.entries(savedAttractions)) {
        ...
        const localMorningArr = localSavedAttractions[slide].morning;
        const localAfternoonArr = localSavedAttractions[slide].afternoon;
        const localEveningArr = localSavedAttractions[slide].evening;
        
        const savedMorningArr = morning; // savedAttractions[slide].morning;
        const savedAfternoonArr = afternoon; // savedAttractions[slide].afternoon;
        const savedEveningArr = evening; // savedAttractions[slide].evening;
        
        if (slide === 'slide1') {
            slide1MorningArr = [...savedMorningArr, ...localMorningArr];
            slide1AfternoonArr = [...savedAfternoonArr, ...localAfternoonArr];
            slide1EveningArr = [...savedEveningArr, ...localEveningArr];
            
            savedAttractions.slide1.morning = slide1MorningArr;
            savedAttractions.slide1.afternoon = slide1AfternoonArr;
            savedAttractions.slide1.evening = slide1EveningArr;
        }
        else if (slide === 'slide2') {
            slide2MorningArr = [...savedMorningArr, ...localMorningArr];
            slide2AfternoonArr = [...savedAfternoonArr, ...localAfternoonArr];
            slide2EveningArr = [...savedEveningArr, ...localEveningArr];
    
            savedAttractions.slide2.morning = slide2MorningArr;
            savedAttractions.slide2.afternoon = slide2AfternoonArr;
            savedAttractions.slide2.evening = slide2EveningArr;
        }
    }
    
    console.log('localSavedAttractions', localSavedAttractions)
    console.log('savedAttractions 2::', savedAttractions)
...
document.querySelector('[data-ak="travel-details"]').addEventListener('click', e => {
  handleSearchResultsRemoveLocation(e);
});

function handleSearchResultsRemoveLocation(e) {
...
function restoreSavedAttractions(savedAttractions) {
  ...
  for (const [slide, attractions] of Object.entries(savedAttractions)) {
    ...
    if (morning && morning.length) {
      processSectionAttractions(morning, $morningWrap);
    }
    if (afternoon && afternoon.length) {
      processSectionAttractions(afternoon, $afternoonWrap);
    }
    if (evening && evening.length) {
      processSectionAttractions(evening, $eveningWrap);
    }
  }

  $attractionsSliderMask.querySelector('.w-slide .active')?.classList.remove('active');

  function processSectionAttractions(attractions, $sectionWrap) {

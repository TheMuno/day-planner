...
$calculatePasses.addEventListener('click', async e => {
  ...
  populateCityPasses(passData, $citypassName, $citypassPrice, $citypassResultsContainer) //, 'citypass', $citypassName, $citypassPrice, $citypassResultsContainer); 

  function populateCityPasses(passData, $passNameEl, $passPriceEl, $passContainer){ //, 'citypass', $citypassName, $citypassPrice, $citypassResultsContainer) {
    ...
    const requiredAttractions = passData.some(([id, data]) => {
      return data.required_attractions_id.includes('ChIJaXQRs6lZwokRY6EFpJnhNNE')
      && data.required_attractions_id.includes('ChIJCXoPsPRYwokRsV1MYnKBfaI')
    });

    const excludedAttractions = passData.some(([id, data]) => {
      return data.excluded_attractions_id.includes('ChIJ3aqq5Q1ZwokRb9hLO7Gyxgw')
      || data.excluded_attractions_id.includes('ChIJKxDbe_lYwokRVf__s8CPn-o')
    });

    console.log('requiredAttractions:', requiredAttractions)
    console.log('excludedAttractions:', excludedAttractions)

    // if (excludedAttractions.length) return; 
